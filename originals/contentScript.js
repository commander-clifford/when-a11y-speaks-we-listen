...
function collectAndDisplayImages() {
  ...
  const kv = (label, value) =>
    value ? `<div class="kv"><div class="k">${esc(label)}</div><div class="v">${esc(value)}</div></div>` : '';
  ...
  const metaBlock = `
    <div class="page-header md-surface md-elevation-1">
      <div>
        ...
        <div class="kvs">
          ${kv('Lang', meta.lang)}
          ${kv('Robots', meta.robots)}
          ${kv('Viewport', meta.viewport)}
          ${kv('Author', meta.author)}
          ${kv('Generator', meta.generator)}
          ${kv('OG Title', meta.og.title)}
          ${kv('OG Desc', meta.og.description)}
          ${kv('OG Type', meta.og.type)}
          ${kv('Twitter Card', meta.twitter.card)}
          ${kv('Twitter Title', meta.twitter.title)}
          ${kv('Twitter Desc', meta.twitter.description)}
        </div>
