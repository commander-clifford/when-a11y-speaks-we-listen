...
function collectAndDisplayImages() {
  ...
  const metaItem = (label, value, isLink = false) => {
    if (!value) return '';
    const renderedValue = isLink
      ? `<a href="${escAttr(value)}" target="_blank" rel="noopener">${esc(value)}</a>`
      : esc(value);

    return `<li class="meta-item"><span class="meta-label">${esc(label)}</span><span class="meta-value">${renderedValue}</span></li>`;
  };
  ...
  const metaBlock = `
    <div class="page-header md-surface md-elevation-1">
      <div class="page-header-main">
        ...
        <ul class="meta-list">
          ${metaItem('URL', meta.url, true)}
          ${metaItem('Canonical', meta.canonical, true)}
          ${metaItem('Lang', meta.lang)}
          ${metaItem('Robots', meta.robots)}
          ${metaItem('Viewport', meta.viewport)}
          ${metaItem('Author', meta.author)}
          ${metaItem('Generator', meta.generator)}
          ${metaItem('OG Title', meta.og.title)}
          ${metaItem('OG Desc', meta.og.description)}
          ${metaItem('OG Type', meta.og.type)}
          ${metaItem('Twitter Card', meta.twitter.card)}
          ${metaItem('Twitter Title', meta.twitter.title)}
          ${metaItem('Twitter Desc', meta.twitter.description)}
          ${meta.description ? `<li class="meta-item meta-item-description"><span class="meta-label">Description</span><span class="meta-value">${esc(meta.description)}</span></li>` : ''}
        </ul>
